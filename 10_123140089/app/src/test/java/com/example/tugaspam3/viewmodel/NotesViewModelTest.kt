...
class NotesViewModelTest {
    ...
    fun `initial state displays device and battery info`() = runTest {
        val viewModel = NotesViewModel(repository, settingsManager, networkMonitor, deviceInfo, batteryInfo)
        advanceUntilIdle()
    ...
    @Test
    fun `uiState reflects network status change`() = runTest {
        ...
        viewModel.uiState.test {
            ...
            assertEquals(false, awaitItem().isOnline)
        }
    }
    ...
    fun `toggleDarkMode calls settingsManager`() = runTest {
        ...
        coVerify { settingsManager.setDarkMode(any()) }
    }
}
